...
<div class="view view-<?php print $css_name; ?> view-id-<?php print $name; ?> view-display-id-<?php print $display_id; ?> view-dom-id-<?php print $dom_id; ?>">
  ...
  <?php if ($more): ?>
    <?php
      if ($preset = variable_get('stuartc_block_views-'. $view->name .'-'. $view->current_display .'_preset', NULL)) {
        $preset = str_replace('block_title', 'block_more', $preset);
        $more = str_replace(' '. t('more'), theme('textimage_image', $preset, t('more')), $more);
      }
    ?>
    <?php print $more; ?>
  <?php endif; ?>
